...
def find_best_pair(
        pairs_freq: Dict[Tuple[bytes, bytes], int]
) -> Tuple[bytes, bytes]:
    return max(pairs_freq, key=lambda x: (pairs_freq[x], x))

def perform_merge_step(
        vocab: Dict[Tuple[bytes, ...], int],
        pairs_freq: Dict[Tuple[bytes, bytes], int],
...
def train_bpe(
    ...
    word_freqs = run_pretokenization(input_path, special_tokens)
    ...
    for i in range(num_merges):
        ...
        final_vocab_list.append(best_pair[0] + best_pair[1])

        vocab = perform_merge_step(vocab, pairs_freq, best_pair)
